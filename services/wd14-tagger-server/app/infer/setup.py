...
async def download_file(file_name: str, file_url: str, file_dir: str):
    ...
    try:
        ...
        # Handle case where Content-Length header is missing or other download errors
        if 'content-length' in str(e).lower() or isinstance(e, KeyError):
            logger.warning(f"Content-Length header missing for {file_name}, trying alternative download method")
            import httpx
            import pathlib
            file_path = pathlib.Path(file_dir) / file_name
            # Follow redirects and stream download
            with httpx.stream("GET", file_url, follow_redirects=True) as response:
                response.raise_for_status()
                with open(file_path, 'wb') as f:
                    for chunk in response.iter_bytes(chunk_size=8192):
                        f.write(chunk)
            logger.success(f"Download file {file_name} from {file_url} success (alternative method)")
        else:
            raise
